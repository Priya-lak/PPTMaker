...
class PPT:
    ...
    def populate_slide_content(
        ...
        original_slide_placeholders = [pl.name for pl in slide.placeholders]
        logger.debug(
            f"layout: {slide.slide_layout.name}, original slide placeholders {pformat(original_slide_placeholders)}"
        )
        logger.debug(
            f"layout: {sl_layout}, generated {pformat(list(placeholder_content.keys()))}"
        )
        for placeholder in slide.placeholders:
            try:
                ...
                if placeholder_name in placeholder_content:
                    content = placeholder_content[placeholder_name]
    ...
    @staticmethod
    def add_sample_text(slide: Slide):
        """Add sample text to all text placeholders in the slide"""
        for placeholder in slide.placeholders:
            try:
                # Check if placeholder has text frame (is a text placeholder)
                if placeholder.has_text_frame and placeholder.text_frame is not None:
                    # Determine sample text based on placeholder type/name
                    logger.info(f"place holder {placeholder.name} contains text frame")
                    if "title" in placeholder.name.lower():
                        logger.info("title placeholder")
                        placeholder.text = f"Sample Title"
                    elif "subtitle" in placeholder.name.lower():
                        placeholder.text = f"Sample Subtitle - {placeholder.name}"
                    elif (
                        "content" in placeholder.name.lower()
                        or "body" in placeholder.name.lower()
                    ):
                        placeholder.text = f"Sample content for {placeholder.name}.\nThis is a multi-line text example.\n• Bullet point 1\n• Bullet point 2"
                    else:
                        placeholder.text = f"Sample text for {placeholder.name}"
            except Exception as e:
                # Skip placeholders that can't accept text (like image placeholders)
                logger.info(f"Skipped placeholder {placeholder.name}: {e}")
                continue
