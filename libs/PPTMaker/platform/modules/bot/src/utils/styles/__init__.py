...
class StyleFactory:
    ...
    _styles = {
        StylesEnum.PROFESSIONAL: ProfessionalStyle,
        StylesEnum.MINIMALIST: MinimalistStyle,
        StylesEnum.PUNK: PunkStyle,
    ...
    @classmethod
    def create_style(
        cls, theme: StylesEnum, config: Optional[PresentationStyleConfig] = None
    ) -> BasePresentationStyle:
    ...
    @classmethod
    def get_available_themes(cls) -> list[StylesEnum]:
        """Get list of available style themes"""
        return list(cls._styles.keys())

    @classmethod
    def register_style(cls, theme: StylesEnum, style_class: type):
